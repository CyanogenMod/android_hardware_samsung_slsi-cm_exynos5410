...
ExynosCamera::ExynosCamera()
{
    ...
    m_tryPreviewStop2 = true;
    m_tryVideoStop2   = true;
    m_tryPictureStop2 = true;
    ...
    m_isHWVDis = true;
#endif
    m_isFirtstIspStart2 = false;
    m_isFirtstSensorStart2 = false;
    m_sensorFrameCount = 0;
    m_sensorFrameCount2 = 0;
#if CAPTURE_BUF_GET
    ...
    for (int i = 0; i < VIDEO_MAX_FRAME; i++) {
        ...
        previewMetaBuffer2[i] = -1;
        pictureMetaBuffer2[i] = -1;
    }
...
*/
    ...
    m_isFirtstIspStart = true;
    m_isFirtstSensorStart = true;
    m_isFirtstIspStart2 = true;
    m_isFirtstSensorStart2 = true;
...
bool ExynosCamera::destroy(void)
{
    ...
    if (m_camera_info[SENSOR_FRONT].vdiso.flagStart == true)
        stopVdisOutput();
#endif
    if (m_camera_info[SENSOR_FRONT].video.flagStart == true)
        stopVideo2();

    if (m_camera_info[SENSOR_FRONT].preview.flagStart == true)
        stopPreview2();

    if (m_camera_info[SENSOR_FRONT].picture.flagStart == true)
        stopPicture2();

    if (m_camera_info[SENSOR_FRONT].sensor.flagStart == true)
        stopSensor2();

    if (m_camera_info[SENSOR_FRONT].isp.flagStart == true)
        stopIsp2();
...
bool ExynosCamera::startPreview2(void)
{
    int index;
    ...
    CLOGD("[%s] (%d)", __func__, __LINE__);
    CLOGD("(flagStart %d), (%d x %d), format 0x%x", m_camera_info[SENSOR_FRONT].preview.flagStart,
        m_camera_info[SENSOR_FRONT].preview.width,
        m_camera_info[SENSOR_FRONT].preview.height,
        m_camera_info[SENSOR_FRONT].preview.format);

    if (m_camera_info[SENSOR_FRONT].preview.flagStart == false) {
        m_camera_info[SENSOR_FRONT].preview.width  = m_curCameraInfo[SENSOR_FRONT]->previewW;
        m_camera_info[SENSOR_FRONT].preview.height = m_curCameraInfo[SENSOR_FRONT]->previewH;
        m_camera_info[SENSOR_FRONT].preview.format = m_curCameraInfo[SENSOR_FRONT]->previewColorFormat;
        m_camera_info[SENSOR_FRONT].preview.planes = m_curCameraInfo[m_camera_default]->previewBufPlane;
        m_camera_info[SENSOR_FRONT].preview.memory = V4L2_CAMERA_MEMORY_TYPE;
        m_camera_info[SENSOR_FRONT].preview.type   = V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE;
        m_camera_info[SENSOR_FRONT].preview.ionClient = m_ionCameraClient;
        m_camera_info[SENSOR_FRONT].preview.buffers = NUM_PREVIEW_BUFFERS;

        int sensorId = m_getSensorId(m_cameraId);
        sensorId = (0 << REPROCESSING_SHFIT) | ((FIMC_IS_VIDEO_SCP_NUM - FIMC_IS_VIDEO_SEN0_NUM) << VIDEO_INDEX_SHFIT) | (sensorId << 0);

        if (cam_int_s_input(&(m_camera_info[SENSOR_FRONT].preview), sensorId) < 0) {
            CLOGE("ERR(%s):cam_int_s_input(%d) fail", __func__, sensorId);
            return false;
        }

        if (cam_int_s_fmt(&m_camera_info[SENSOR_FRONT].preview) < 0) {
            CLOGE("ERR(%s):cam_int_s_fmt() fail", __func__);
            return false;
        }

        if (cam_int_reqbufs(&m_camera_info[SENSOR_FRONT].preview) < 0) {
            CLOGE("ERR(%s):cam_int_reqbufs() fail", __func__);
            return false;
        }

        ExynosBuffer nullBuf;
        for (int i = 0; i < m_camera_info[SENSOR_FRONT].preview.buffers; i++) {
            m_camera_info[SENSOR_FRONT].preview.buffer[i] = nullBuf;
            m_camera_info[SENSOR_FRONT].preview.buffer[i].size.extS[0] = m_camera_info[SENSOR_FRONT].preview.width * m_camera_info[SENSOR_FRONT].preview.height;
            m_camera_info[SENSOR_FRONT].preview.buffer[i].size.extS[1] = m_camera_info[SENSOR_FRONT].preview.width * m_camera_info[SENSOR_FRONT].preview.height/2;

            if (allocMem(m_camera_info[SENSOR_FRONT].preview.ionClient, &m_camera_info[SENSOR_FRONT].preview.buffer[i], 0) == false) {
        ...
        // for frame sync
        for (int i = 0; i < VIDEO_MAX_FRAME; i++) {
            if (m_camera_info[SENSOR_FRONT].preview.buffer[i].virt.p != NULL ||
                m_camera_info[SENSOR_FRONT].preview.buffer[i].phys.p != 0) {
                m_camera_info[SENSOR_FRONT].preview.buffer[i].size.extS[m_camera_info[SENSOR_FRONT].preview.planes - 1]
                    = ALIGN(META_DATA_SIZE, PAGE_SIZE);

                if (allocMemSinglePlane(m_ionCameraClient,
                                        &m_camera_info[SENSOR_FRONT].preview.buffer[i],
                                        m_camera_info[SENSOR_FRONT].preview.planes - 1,
                                        true) == false) {
                    CLOGE("ERR(%s):m_allocCameraMemorySingle() fail", __func__);
                    return false;
                } else {
                    memset(m_camera_info[SENSOR_FRONT].preview.buffer[i].virt.extP[m_camera_info[SENSOR_FRONT].preview.planes - 1],
                            0, m_camera_info[SENSOR_FRONT].preview.buffer[i].size.extS[m_camera_info[SENSOR_FRONT].preview.planes - 1]);
                }
                previewMetaBuffer2[i] = m_camera_info[SENSOR_FRONT].preview.buffer[i].fd.extFd[m_camera_info[SENSOR_FRONT].preview.planes - 1];
            }
        }

        m_camera_info[SENSOR_FRONT].dummy_shot.request_scp = 1;

        for (int i = 0; i < m_camera_info[SENSOR_FRONT].preview.buffers; i++) {
            if (cam_int_qbuf(&(m_camera_info[SENSOR_FRONT].preview), i) < 0) {
                CLOGE("ERR(%s):cam_int_qbuf(%d) failed", __func__, i);
                return false;
            }
        }

        if (cam_int_streamon(&(m_camera_info[SENSOR_FRONT].preview)) < 0) {
            CLOGE("ERR(%s):cam_int_streamon(preview2) fail", __func__);
            return false;
        }

        // This from startIsp
        if (cam_int_streamon(&(m_camera_info[SENSOR_FRONT].isp)) < 0) {
            CLOGE("ERR(%s):cam_int_streamon(isp) fail", __func__);
            return false;
        }

        m_tryPreviewStop2 = false;
        m_camera_info[SENSOR_FRONT].preview.flagStart = true;
...
bool ExynosCamera::stopPreview2(void)
{
    if (m_flagCreate == false) {
        CLOGE("ERR(%s):Not yet Created", __func__);
        return false;
    ...
    if (m_camera_info[SENSOR_FRONT].preview.flagStart == true) {
        ...
        // skip stopPreview
        if ((m_camera_info[SENSOR_FRONT].preview.fd == m_camera_info[SENSOR_FRONT].video.fd && m_tryVideoStop2 == false) ||
            (m_camera_info[SENSOR_FRONT].preview.fd == m_camera_info[SENSOR_FRONT].picture.fd && m_tryPictureStop2 == false)) {
            CLOGD("DEBUG(%s):stopPreview is skipped...", __func__);
            return true;
        }
        ...
        if (cam_int_streamoff(&m_camera_info[SENSOR_FRONT].preview) < 0) {
            CLOGE("ERR(%s):exynos_v4l2_streamoff() fail", __func__);
            return false;
        }

        m_camera_info[SENSOR_FRONT].dummy_shot.request_scp = 0;

        if (0 < m_camera_info[SENSOR_FRONT].preview.buffers) {
            int buffers = m_camera_info[SENSOR_FRONT].preview.buffers;

            m_camera_info[SENSOR_FRONT].preview.buffers = 0;
            if (cam_int_reqbufs(&m_camera_info[SENSOR_FRONT].preview) < 0) {
                CLOGE("ERR(%s):cam_int_reqbufs() fail", __func__);
                return false;
            }
            ...
            for(int i = 0; i < buffers; i++)
                freeMem(&m_camera_info[SENSOR_FRONT].preview.buffer[i]);
        }

        // for frame sync
        for (int i = 0; i < VIDEO_MAX_FRAME; i++) {
            freeMemSinglePlane(&m_camera_info[SENSOR_FRONT].preview.buffer[i],
                                          m_camera_info[SENSOR_FRONT].preview.planes - 1);
        }
...
bool ExynosCamera::flagStartPreview2(void)
{
    return m_camera_info[SENSOR_FRONT].preview.flagStart;
}
...
bool ExynosCamera::getPreviewBuf2(ExynosBuffer *buf, bool *isValid)
{
    int index = 0;
    struct camera2_stream *shot_ext;
    ...
    index = cam_int_dqbuf(&(m_camera_info[SENSOR_FRONT].preview));
    if (index < 0) {
        CLOGE("ERR(%s):cam_int_dqbuf() on preview fail", __func__);
        return false;
    }

    *buf = m_camera_info[SENSOR_FRONT].preview.buffer[index];
    buf->reserved.p = index;

    shot_ext = (struct camera2_stream *)(m_camera_info[SENSOR_BACK].preview.buffer[index].virt.extP[m_curCameraInfo[m_camera_default]->previewBufPlane - 1]);
    *isValid = shot_ext->fvalid ? true : false;

    return true;
}

bool ExynosCamera::putPreviewBuf2(ExynosBuffer *buf)
{
    if (m_flagCreate == false) {
        CLOGE("ERR(%s):Not yet created fail", __func__);
        return false;
    ...
    if (cam_int_qbuf(&(m_camera_info[SENSOR_FRONT].preview), buf->reserved.p) < 0) {
        CLOGE("ERR(%s):cam_int_qbuf(%d) failed", __func__, buf->reserved.p);
        return false;
    }
...
bool ExynosCamera::startSensor2(void)
{
    if (m_flagCreate == false) {
        CLOGE("ERR(%s):Not yet Created", __func__);
        return false;
    ...
    CLOGD("[%s] (%d)", __func__, __LINE__);
    CLOGD("(flagStart %d), (%d x %d), format 0x%x", m_camera_info[SENSOR_FRONT].sensor.flagStart,
        m_camera_info[SENSOR_FRONT].sensor.width,
        m_camera_info[SENSOR_FRONT].sensor.height,
        m_camera_info[SENSOR_FRONT].sensor.format);

    if (m_camera_info[SENSOR_FRONT].sensor.flagStart == false) {

        Mutex::Autolock lock(m_sensorLock2);

        m_camera_info[SENSOR_FRONT].sensor.width   = m_curCameraInfo[SENSOR_FRONT]->pictureW + 16;
        m_camera_info[SENSOR_FRONT].sensor.height  = m_curCameraInfo[SENSOR_FRONT]->pictureH + 10;
        m_camera_info[SENSOR_FRONT].sensor.format  = V4L2_PIX_FMT_SBGGR12;
        m_camera_info[SENSOR_FRONT].sensor.planes  = 2;
        m_camera_info[SENSOR_FRONT].sensor.buffers = NUM_BAYER_BUFFERS;
        m_camera_info[SENSOR_FRONT].sensor.memory  = V4L2_CAMERA_MEMORY_TYPE;
        m_camera_info[SENSOR_FRONT].sensor.type    = V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE;
        m_camera_info[SENSOR_FRONT].sensor.ionClient = m_ionCameraClient;

        ExynosBuffer nullBuf;

        for (int i = 0; i < m_camera_info[SENSOR_FRONT].sensor.buffers; i++) {
            m_camera_info[SENSOR_FRONT].sensor.buffer[i] = nullBuf;
            m_camera_info[SENSOR_FRONT].sensor.buffer[i].size.extS[0] = (m_curCameraInfo[SENSOR_FRONT]->pictureW + 16) * (m_curCameraInfo[SENSOR_FRONT]->pictureH + 10) * 2;
            m_camera_info[SENSOR_FRONT].sensor.buffer[i].size.extS[1] = META_DATA_SIZE;

            if (allocMem(m_camera_info[SENSOR_FRONT].sensor.ionClient, &m_camera_info[SENSOR_FRONT].sensor.buffer[i], 1 << 1) == false) {
                CLOGE("ERR(%s):allocMem() fail", __func__);
                return false;
            } else {
                memset(m_camera_info[SENSOR_FRONT].sensor.buffer[i].virt.extP[1],
                        0, m_camera_info[SENSOR_FRONT].sensor.buffer[i].size.extS[1]);
            }
        }

        if (m_isFirtstSensorStart2 == true) {
            int sensorId = m_getSensorId(m_cameraId);
            sensorId = (0 << REPROCESSING_SHFIT) | ((FIMC_IS_VIDEO_SEN0_NUM - FIMC_IS_VIDEO_SEN0_NUM) << VIDEO_INDEX_SHFIT) | (sensorId << 0);

            if (cam_int_s_input(&(m_camera_info[SENSOR_FRONT].sensor), sensorId) < 0) {
                CLOGE("ERR(%s):cam_int_s_input(%d) fail", __func__, sensorId);
                return false;
            }
            m_isFirtstSensorStart2 = false;
        ...
        if (cam_int_s_fmt(&(m_camera_info[SENSOR_FRONT].sensor)) < 0) {
            CLOGE("ERR(%s): sensor s_fmt fail",  __func__);
            return false;
        }

        if (cam_int_reqbufs(&(m_camera_info[SENSOR_FRONT].sensor)) < 0) {
            CLOGE("ERR(%s):cam_int_reqbufs() fail", __func__);
            return false;
        }
        ...
        m_camera_info[SENSOR_FRONT].dummy_shot.request_3ax = 1;
        m_camera_info[SENSOR_FRONT].dummy_shot.request_isp = 1;
        m_camera_info[SENSOR_FRONT].dummy_shot.request_scp = 1;
        m_camera_info[SENSOR_FRONT].dummy_shot.request_scc = 1;
        m_camera_info[SENSOR_FRONT].dummy_shot.request_scc = 0;

        for (int i = 0; i < NUM_MIN_SENSOR_QBUF; i++) {
            memcpy(m_camera_info[SENSOR_FRONT].sensor.buffer[i].virt.extP[1], &(m_camera_info[SENSOR_FRONT].dummy_shot), sizeof(camera2_shot_ext));
            m_camera_info[SENSOR_FRONT].sensor.buffer[i].reserved.extP[FRAME_COUNT_INDEX] = m_sensorFrameCount2++;

            if (cam_int_qbuf(&(m_camera_info[SENSOR_FRONT].sensor), i) < 0) {
                CLOGE("ERR(%s):cam_int_qbuf(%d) failed", __func__, i);
                return false;
            }
        }

        for (int i = NUM_MIN_SENSOR_QBUF; i < m_camera_info[SENSOR_FRONT].sensor.buffers; i++)
            m_pushSensorQ2(i);

        if (cam_int_streamon(&(m_camera_info[SENSOR_FRONT].sensor)) < 0) {
            CLOGE("ERR(%s):cam_int_streamon(sensor2) fail", __func__);
            return false;
        }

        m_camera_info[SENSOR_FRONT].sensor.flagStart = true;
    }

    return true;
}

bool ExynosCamera::stopSensor2(void)
{
    if (m_flagCreate == false) {
        CLOGE("ERR(%s):Not yet Created", __func__);
        return false;
    ...
    if (m_camera_info[SENSOR_FRONT].sensor.flagStart == true) {
        ...
        if (cam_int_streamoff(&m_camera_info[SENSOR_FRONT].sensor) < 0) {
            CLOGE("ERR(%s):exynos_v4l2_streamoff() fail", __func__);
            return false;
        }

        m_camera_info[SENSOR_FRONT].dummy_shot.request_3ax = 0;
        m_camera_info[SENSOR_FRONT].dummy_shot.request_isp = 0;

        if (0 < m_camera_info[SENSOR_FRONT].sensor.buffers) {
            int buffers = m_camera_info[SENSOR_FRONT].sensor.buffers;

            m_camera_info[SENSOR_FRONT].sensor.buffers = 0;
            if (cam_int_reqbufs(&m_camera_info[SENSOR_FRONT].sensor) < 0) {
                CLOGE("ERR(%s):cam_int_reqbufs() fail", __func__);
                return false;
            }
            ...
            for (int i = 0; i < buffers; i++)
                freeMem(&m_camera_info[SENSOR_FRONT].sensor.buffer[i]);

            for (int i = 0; i < VIDEO_MAX_FRAME; i++)
                freeMemSinglePlane(&m_camera_info[SENSOR_FRONT].sensor.buffer[i], m_camera_info[SENSOR_FRONT].sensor.planes - 1);
        }
...
bool ExynosCamera::flagStartSensor2(void)
{
    return m_camera_info[SENSOR_FRONT].sensor.flagStart;
}

bool ExynosCamera::getSensorBuf2(ExynosBuffer *buf)
{
    if (m_flagCreate == false) {
        CLOGE("ERR(%s):Not yet Created", __func__);
        return false;
    ...
    while (1) {
        /* sensor dq */
        index_sensor = cam_int_dqbuf(&(m_camera_info[SENSOR_FRONT].sensor));
        if (index_sensor < 0) {
            CLOGE("ERR(%s):cam_int_dqbuf() on sensor fail", __func__);
            return false;
        }
        m_pushSensorQ2(index_sensor);

        *buf = m_camera_info[SENSOR_FRONT].sensor.buffer[index_sensor];
        buf->reserved.p = index_sensor;
        ...
        /* sensor q */
        index_sensor = m_popSensorQ2();
        if (index_sensor < 0) {
            CLOGE("ERR(%s):m_popSensorQ2() fail", __func__);
            return false;
        }

        CLOGD("DEBUG(%s):(%d), index_sensor %d -- Dropped", __func__, __LINE__, index_sensor);
        if (cam_int_qbuf(&(m_camera_info[SENSOR_FRONT].sensor), index_sensor) < 0) {
            CLOGE("ERR(%s):cam_int_qbuf(%d) on sensor failed", __func__, index_sensor);
            return false;
        }
    }
...
bool ExynosCamera::putSensorBuf2(ExynosBuffer *buf)
{
    if (m_flagCreate == false) {
        CLOGE("ERR(%s):Not yet Created", __func__);
        return false;
    ...
    if (m_camera_info[SENSOR_FRONT].sensor.flagStart == false) {
        CLOGE("ERR(%s):Not yet sensor2 started fail", __func__);
        return false;
    }
    ...
    index_sensor = m_popSensorQ2();
    if (index_sensor < 0) {
        CLOGW("WARN(%s):m_popSensorQ2() fail", __func__);
        return true;
    }

    shot_ext = (struct camera2_shot_ext *)(m_camera_info[SENSOR_FRONT].sensor.buffer[index_sensor].virt.extP[1]);

    shot_ext->setfile = m_camera_info[SENSOR_FRONT].dummy_shot.setfile;
    shot_ext->request_3ax = m_camera_info[SENSOR_FRONT].dummy_shot.request_3ax;
    shot_ext->request_isp = m_camera_info[SENSOR_FRONT].dummy_shot.request_isp;
    shot_ext->request_scc = m_camera_info[SENSOR_FRONT].dummy_shot.request_scc;
    shot_ext->request_scp = m_camera_info[SENSOR_FRONT].dummy_shot.request_scp;
    shot_ext->request_dis = m_camera_info[SENSOR_FRONT].dummy_shot.request_dis;

    shot_ext->dis_bypass = m_camera_info[SENSOR_FRONT].dummy_shot.dis_bypass;
    shot_ext->dnr_bypass = m_camera_info[SENSOR_FRONT].dummy_shot.dnr_bypass;
    shot_ext->fd_bypass = m_camera_info[SENSOR_FRONT].dummy_shot.fd_bypass;
    shot_ext->shot.magicNumber= m_camera_info[SENSOR_FRONT].dummy_shot.shot.magicNumber;
    ...
    m_camera_info[SENSOR_FRONT].sensor.buffer[index_sensor].reserved.extP[FRAME_COUNT_INDEX] = m_sensorFrameCount2++;

    return true;
}

bool ExynosCamera::getISPBuf2(ExynosBuffer *buf)
{
    if (m_flagCreate == false) {
        CLOGE("ERR(%s):Not yet Created", __func__);
        return false;
    ...
    if (m_camera_info[SENSOR_FRONT].isp.flagStart == false) {
        CLOGE("ERR(%s):Not yet isp started fail", __func__);
        return false;
    }
    ...
    int index_isp = 0;
    camera2_shot_ext *shot_ext;
    index_isp = cam_int_dqbuf(&(m_camera_info[SENSOR_FRONT].isp));
    if (index_isp < 0) {
        CLOGE("ERR(%s):cam_int_dqbuf() on isp fail", __func__);
        return false;
    }

    shot_ext = (struct camera2_shot_ext *)m_camera_info[SENSOR_FRONT].isp.buffer[index_isp].virt.extP[1];

    if (shot_ext->request_isp == 0) {
        /* TODO: Error handling ISP shot done invalid */
        m_traceCount = TRACE_COUNT;
        CLOGE("ERR(%s): isp Shot done is invalid(expected request_isp is %d, but we got %d), skip frame(%d)",
                __func__, m_camera_info[SENSOR_FRONT].dummy_shot.request_isp, shot_ext->request_isp, shot_ext->shot.dm.request.frameCount);
    }

    /* back-up isp_dm : result of isp metadata */
    memcpy(&m_camera_info[SENSOR_FRONT].isp_dm.shot.dm.stats, &shot_ext->shot.dm.stats, sizeof(struct camera2_stats_dm));

    return true;
}

bool ExynosCamera::putISPBuf2(ExynosBuffer *buf)
{
    if (m_flagCreate == false) {
        CLOGE("ERR(%s):Not yet Created", __func__);
        return false;
    ...
    shot_ext->shot.ctl.request.frameCount = m_camera_info[SENSOR_FRONT].isp.buffer[index_isp].reserved.extP[FRAME_COUNT_INDEX];

    /*  isp q */
    if (cam_int_qbuf(&(m_camera_info[SENSOR_FRONT].isp), index_isp) < 0) {
        CLOGE("ERR(%s):cam_int_qbuf(%d) on isp failed", __func__, index_isp);
        return false;
    }
...
bool ExynosCamera::StartStream2()
{
    CLOGD("[%s] (%d)", __func__, __LINE__);

    if (exynos_v4l2_s_ctrl(m_camera_info[SENSOR_FRONT].sensor.fd,
        V4L2_CID_IS_S_STREAM, IS_ENABLE_STREAM) < 0) {
        CLOGE("ERR(%s):exynos_v4l2_s_ctrl fail(IS_ENABLE_STREAM)", __func__);
        return false;
    }
...
bool ExynosCamera::startIsp2(void)
{
    CLOGD("[%s] (%d)", __func__, __LINE__);
    CLOGD("(flagStart %d), (%d x %d), format 0x%x", m_camera_info[SENSOR_FRONT].isp.flagStart,
        m_camera_info[SENSOR_FRONT].isp.width,
        m_camera_info[SENSOR_FRONT].isp.height,
        m_camera_info[SENSOR_FRONT].isp.format);

    if (m_camera_info[SENSOR_FRONT].isp.flagStart == false) {
        m_camera_info[SENSOR_FRONT].isp.width   = m_curCameraInfo[SENSOR_FRONT]->ispW;
        m_camera_info[SENSOR_FRONT].isp.height  = m_curCameraInfo[SENSOR_FRONT]->ispH;
        m_camera_info[SENSOR_FRONT].isp.format  = V4L2_PIX_FMT_SBGGR12;
        m_camera_info[SENSOR_FRONT].isp.planes  = 2;
        m_camera_info[SENSOR_FRONT].isp.buffers = NUM_BAYER_BUFFERS;
        m_camera_info[SENSOR_FRONT].isp.memory  = V4L2_CAMERA_MEMORY_TYPE;
        m_camera_info[SENSOR_FRONT].isp.type    = V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE;
        m_camera_info[SENSOR_FRONT].isp.ionClient = m_ionCameraClient;

        ExynosBuffer nullBuf;

        for (int i = 0; i < m_camera_info[SENSOR_FRONT].isp.buffers; i++) {
            m_camera_info[SENSOR_FRONT].isp.buffer[i] = nullBuf;
            m_camera_info[SENSOR_FRONT].isp.buffer[i].size.extS[0] = m_curCameraInfo[SENSOR_FRONT]->ispW * m_curCameraInfo[SENSOR_FRONT]->ispH * 2;
            m_camera_info[SENSOR_FRONT].isp.buffer[i].size.extS[1] = META_DATA_SIZE; /* This is the predefined value by driver. */

            if (allocMem(m_camera_info[SENSOR_FRONT].isp.ionClient, &m_camera_info[SENSOR_FRONT].isp.buffer[i], 1 << 1) == false) {
                CLOGE("ERR(%s):allocMem() fail", __func__);
                return false;
            } else {
                memset(m_camera_info[SENSOR_FRONT].isp.buffer[i].virt.extP[1],
                        0, m_camera_info[SENSOR_FRONT].isp.buffer[i].size.extS[1]);
            }
        }

        // Reset Dummy Shot's request
        m_camera_info[SENSOR_FRONT].default_shot.request_3ax = 1;
        m_camera_info[SENSOR_FRONT].default_shot.request_isp = 1;
        m_camera_info[SENSOR_FRONT].default_shot.request_scc = 1;
        m_camera_info[SENSOR_FRONT].default_shot.request_scp = 1;
        m_camera_info[SENSOR_FRONT].default_shot.request_dis = 0;
        ...
        if (m_isFirtstIspStart2 == true) {
            ...
            if (cam_int_s_input(&(m_camera_info[SENSOR_FRONT].isp), sensorId) < 0) {
                CLOGE("ERR(%s):cam_int_s_input(%d) fail", __func__, sensorId);
                return false;
            }
            m_isFirtstIspStart2 = false;
        ...
        if (cam_int_s_fmt(&(m_camera_info[SENSOR_FRONT].isp)) < 0) {
            CLOGE("ERR(%s):cam_int_s_fmt() fail", __func__);
            return false;
        }

        if (cam_int_reqbufs(&(m_camera_info[SENSOR_FRONT].isp)) < 0) {
            CLOGE("ERR(%s):cam_int_reqbufs() fail", __func__);
            return false;
        }

        m_camera_info[SENSOR_FRONT].isp.flagStart = true;
    }

    return true;
}

bool ExynosCamera::stopIsp2(void)
{
    CLOGD("[%s] (%d)", __func__, __LINE__);

    if (m_camera_info[SENSOR_FRONT].isp.flagStart == true) {

        if (cam_int_streamoff(&m_camera_info[SENSOR_FRONT].isp) < 0) {
            CLOGE("ERR(%s):exynos_v4l2_streamoff() fail", __func__);
            return false;
        }

        // This from stopSensor
        if (exynos_v4l2_s_ctrl(m_camera_info[SENSOR_FRONT].sensor.fd, V4L2_CID_IS_S_STREAM, IS_DISABLE_STREAM) < 0)
            CLOGE("ERR(%s):exynos_v4l2_s_ctrl() fail", __func__);

        if (0 < m_camera_info[SENSOR_FRONT].isp.buffers) {
            int buffers = m_camera_info[SENSOR_FRONT].isp.buffers;

            m_camera_info[SENSOR_FRONT].isp.buffers = 0;
            if (cam_int_reqbufs(&m_camera_info[SENSOR_FRONT].isp) < 0) {
                CLOGE("ERR(%s):cam_int_reqbufs() fail", __func__);
                return false;
            }
            ...
            for(int i = 0; i < buffers; i++)
                freeMem(&m_camera_info[SENSOR_FRONT].isp.buffer[i]);
        }
        m_camera_info[SENSOR_FRONT].isp.flagStart = false;
    }
...
bool ExynosCamera::flagStartIsp2(void)
{
    return m_camera_info[SENSOR_FRONT].isp.flagStart;
}
...
bool ExynosCamera::startVideo2(void)
{
    if (m_flagCreate == false) {
        CLOGE("ERR(%s):Not yet Created", __func__);
        return false;
...
#ifdef USE_3DNR_DMAOUT
    if (m_camera_info[SENSOR_FRONT].video.flagStart == false) {
        for (int i = 0; i < VIDEO_MAX_FRAME; i++) {
            if (m_camera_info[SENSOR_FRONT].video.buffer[i].virt.p != NULL ||
                m_camera_info[SENSOR_FRONT].video.buffer[i].phys.p != 0) {
                if (cam_int_qbuf(&m_camera_info[SENSOR_FRONT].video, i) < 0) {
                    CLOGE("ERR(%s):cam_int_qbuf(%d) fail", __func__, i);
                    return false;
                }
            }
        ...
        if (exynos_v4l2_streamon(m_camera_info[SENSOR_FRONT].video.fd, V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) < 0) {
            CLOGE("ERR(%s):exynos_v4l2_streamon() fail", __func__);
            return false;
        }

        m_tryVideoStop2 = false;
        m_camera_info[SENSOR_FRONT].video.flagStart = true;
    }
#endif
...
bool ExynosCamera::stopVideo2(void)
{
    if (m_flagCreate == false) {
        CLOGE("ERR(%s):Not yet Created", __func__);
        return false;
    ...
    if (m_camera_info[SENSOR_FRONT].video.flagStart == true) {
        ...
        // skip stopVideo
        if ((m_camera_info[SENSOR_FRONT].video.fd == m_camera_info[SENSOR_FRONT].preview.fd && m_tryPreviewStop2 == false) ||
            (m_camera_info[SENSOR_FRONT].video.fd == m_camera_info[SENSOR_FRONT].picture.fd && m_tryPictureStop2 == false)) {
            CLOGD("DEBUG(%s):stopVideo is skipped...", __func__);
            return true;
        }

        if (exynos_v4l2_streamoff(m_camera_info[SENSOR_FRONT].video.fd, V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) < 0) {
            CLOGE("ERR(%s):exynos_v4l2_streamoff() fail", __func__);
            return false;
        }
        struct v4l2_requestbuffers req;
        ...
        if (exynos_v4l2_reqbufs(m_camera_info[SENSOR_FRONT].video.fd, &req) < 0) {
            CLOGE("ERR(%s):exynos_v4l2_reqbufs() fail", __func__);
            return false;
        }
...
bool ExynosCamera::startPicture2(void)
{
    if (m_flagCreate == false) {
        CLOGE("ERR(%s):Not yet Created", __func__);
        return false;
    ...
    if (m_camera_info[SENSOR_FRONT].picture.flagStart == false) {
        m_camera_info[SENSOR_FRONT].picture.width   = m_curCameraInfo[SENSOR_FRONT]->pictureW;
        m_camera_info[SENSOR_FRONT].picture.height  = m_curCameraInfo[SENSOR_FRONT]->pictureH;
        m_camera_info[SENSOR_FRONT].picture.format  = m_curCameraInfo[SENSOR_FRONT]->pictureColorFormat;
        m_camera_info[SENSOR_FRONT].picture.planes  = NUM_CAPTURE_PLANE;
        m_camera_info[SENSOR_FRONT].picture.buffers = NUM_PICTURE_BUFFERS;
        m_camera_info[SENSOR_FRONT].picture.memory  = V4L2_CAMERA_MEMORY_TYPE;
        m_camera_info[SENSOR_FRONT].picture.type    = V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE;
        m_camera_info[SENSOR_FRONT].picture.ionClient = m_ionCameraClient;

        // for frame sync
        for (int i = 0; i < VIDEO_MAX_FRAME; i++) {
            if (m_camera_info[SENSOR_FRONT].picture.buffer[i].virt.p != NULL ||
                m_camera_info[SENSOR_FRONT].picture.buffer[i].phys.p != 0) {
                m_camera_info[SENSOR_FRONT].picture.buffer[i].size.extS[m_camera_info[SENSOR_FRONT].picture.planes - 1]
                    = ALIGN(META_DATA_SIZE, PAGE_SIZE);

                if (allocMemSinglePlane(m_ionCameraClient,
                                        &m_camera_info[SENSOR_FRONT].picture.buffer[i],
                                        m_camera_info[SENSOR_FRONT].picture.planes - 1,
                                        true) == false) {
                    CLOGE("ERR(%s):m_allocCameraMemorySingle() fail", __func__);
                    return false;
                } else {
                    memset(m_camera_info[SENSOR_FRONT].picture.buffer[i].virt.extP[m_camera_info[SENSOR_FRONT].picture.planes - 1],
                            0, m_camera_info[SENSOR_FRONT].picture.buffer[i].size.extS[m_camera_info[SENSOR_FRONT].picture.planes - 1]);
                }
            }
        }
        ...
        if (cam_int_s_input(&(m_camera_info[SENSOR_FRONT].picture), sensorId) < 0) {
            CLOGE("ERR(%s):cam_int_s_input(%d) fail", __func__, sensorId);
            return false;
        }

        if (cam_int_s_fmt(&(m_camera_info[SENSOR_FRONT].picture)) < 0) {
            CLOGE("ERR(%s):cam_int_s_fmt() fail", __func__);
            return false;
        }

        if (cam_int_reqbufs(&(m_camera_info[SENSOR_FRONT].picture)) < 0) {
            CLOGE("ERR(%s):cam_int_reqbufs() fail", __func__);
            return false;
        }
        ...
        for (int i = 0; i < VIDEO_MAX_FRAME; i++) {
            if (m_camera_info[SENSOR_FRONT].picture.buffer[i].virt.p != NULL ||
                m_camera_info[SENSOR_FRONT].picture.buffer[i].phys.p != 0) {
                if (cam_int_qbuf(&m_camera_info[SENSOR_FRONT].picture, i) < 0) {
                    CLOGE("ERR(%s):cam_int_qbuf(%d) fail", __func__, i);
                    return false;
                }
            }
        ...
        if (cam_int_streamon(&(m_camera_info[SENSOR_FRONT].picture)) < 0) {
            CLOGE("ERR(%s):cam_int_streamon() fail", __func__);
            return false;
        }

        m_tryPictureStop2 = false;
        m_camera_info[SENSOR_FRONT].picture.flagStart = true;
    }

    return true;
}

bool ExynosCamera::stopPicture2(void)
{
    int index;
    camera2_shot_ext *shot_ext;
    ...
    if (m_camera_info[SENSOR_FRONT].picture.flagStart == true) {
        ...
        // skip stopPicture
        if ((m_camera_info[SENSOR_FRONT].picture.fd == m_camera_info[SENSOR_FRONT].preview.fd && m_tryPreviewStop2 == false) ||
            (m_camera_info[SENSOR_FRONT].picture.fd == m_camera_info[SENSOR_FRONT].video.fd && m_tryVideoStop2 == false)) {
            CLOGD("DEBUG(%s):stopPicture2 is skipped...", __func__);
            return true;
        }

        if (cam_int_streamoff(&m_camera_info[SENSOR_FRONT].picture) < 0) {
            CLOGE("ERR(%s):exynos_v4l2_streamoff() fail", __func__);
            return false;
        }
        ...
        if (0 < m_camera_info[SENSOR_FRONT].picture.buffers) {
            int buffers = m_camera_info[SENSOR_FRONT].picture.buffers;

            m_camera_info[SENSOR_FRONT].picture.buffers = 0;
            if (cam_int_reqbufs(&m_camera_info[SENSOR_FRONT].picture) < 0) {
                CLOGE("ERR(%s):cam_int_reqbufs() fail", __func__);
                return false;
            }
        ...
        // for frame sync
        for (int i = 0; i < VIDEO_MAX_FRAME; i++) {
            freeMemSinglePlane(&m_camera_info[SENSOR_FRONT].picture.buffer[i],
                                          m_camera_info[SENSOR_FRONT].picture.planes - 1);
        }
...
void ExynosCamera::pictureOn2(void)
{
    m_camera_info[SENSOR_FRONT].dummy_shot.request_scc = 1;
    return;
}

bool ExynosCamera::flagStartPicture2(void)
{
    return m_camera_info[SENSOR_FRONT].picture.flagStart;
}

bool ExynosCamera::setPictureBuf2(ExynosBuffer *buf)
{
    if (m_flagCreate == false) {
        CLOGE("ERR(%s):Not yet created fail", __func__);
        return false;
    ...
    m_camera_info[SENSOR_FRONT].picture.buffer[buf->reserved.p] = *buf;

    return true;
}

bool ExynosCamera::getPictureBuf2(ExynosBuffer *buf)
{
    int index = 0;
...
#ifdef DUMP_BAYER_IMAGE
    if (m_jpegQuality <= 75) {
        ExynosBuffer bayerBuf = m_camera_info.sensor.buffer[m_sensorFrameCount % NUM_BAYER_BUFFERS];

        if (this->fileDump("/data/bayer.raw",
                           bayerBuf.virt.extP[0],
                           m_curCameraInfo[m_camera_default]->pictureW + 16,
                           m_curCameraInfo[m_camera_default]->pictureH + 10,
                           bayerBuf.size.extS[0]) == false) {
            CLOGE("ERR(%s):fileDump(%s) fail", __func__, "/data/bayer.raw");
        }

        if (this->fileDump("/data/bayer.meta",
                            bayerBuf.virt.extP[1],
                            m_curCameraInfo[m_camera_default]->pictureW + 16,
                            m_curCameraInfo[m_camera_default]->pictureH + 10,
                            bayerBuf.size.extS[1]) == false) {
            CLOGE("ERR(%s):fileDump(%s) fail", __func__, "/data/bayer.meta");
        }
    }
#endif

    index = cam_int_dqbuf(&(m_camera_info[SENSOR_FRONT].picture));
    if (index < 0) {
        CLOGE("ERR(%s):cam_int_dqbuf() on picture fail", __func__);
        return false;
    }

    *buf = m_camera_info[SENSOR_FRONT].picture.buffer[index];
    buf->reserved.p = index;
...
bool ExynosCamera::putPictureBuf2(ExynosBuffer *buf)
{
    if (m_flagCreate == false) {
        CLOGE("ERR(%s):Not yet created fail", __func__);
        return false;
    ...
    if (cam_int_qbuf(&(m_camera_info[SENSOR_FRONT].picture), buf->reserved.p) < 0) {
        CLOGE("ERR(%s):cam_int_qbuf(%d) failed", __func__, buf->reserved.p);
        return false;
    }
...
bool ExynosCamera::getPictureSize2(int *w, int *h)
{
    *w = m_curCameraInfo[SENSOR_FRONT]->pictureW;
    *h = m_curCameraInfo[SENSOR_FRONT]->pictureH;
    return true;
}
...
bool ExynosCamera::getPreviewSize2(int *w, int *h)
{
    *w = m_curCameraInfo[SENSOR_FRONT]->previewW;
    *h = m_curCameraInfo[SENSOR_FRONT]->previewH;
    return true;
}
...
bool ExynosCamera::initSensor2()
{
    CLOGD("[%s] (%d) fd %d", __func__, __LINE__, m_camera_info[m_camera_default].sensor.fd);

    /* Init Dummy and Initialized camera_shot */
    memset(&m_camera_info[SENSOR_FRONT].default_shot, 0x00, sizeof(struct camera2_shot_ext));
    memset(&m_camera_info[SENSOR_FRONT].dummy_shot, 0x00, sizeof(struct camera2_shot_ext));
    memset(&m_camera_info[SENSOR_FRONT].is3aa_dm, 0x00, sizeof(struct camera2_shot_ext));
    memset(&m_camera_info[SENSOR_FRONT].isp_dm, 0x00, sizeof(struct camera2_shot_ext));

    struct camera2_shot *shot = &m_camera_info[SENSOR_FRONT].default_shot.shot;
    ...
    /* lens */
    shot->ctl.lens.focusDistance = 0.0f;
    shot->ctl.lens.aperture = (float)m_curCameraInfo[SENSOR_FRONT]->apertureNum / (float)m_curCameraInfo[SENSOR_FRONT]->apertureDen;
    shot->ctl.lens.focalLength = (float)m_curCameraInfo[SENSOR_FRONT]->focalLengthNum / (float)m_curCameraInfo[SENSOR_FRONT]->focalLengthDen;
    shot->ctl.lens.filterDensity = 0.0f;
    shot->ctl.lens.opticalStabilizationMode = ::OPTICAL_STABILIZATION_MODE_OFF;
    ...
    /* scaler */
    if (m_setZoom(0,
                  m_defaultCameraInfo[SENSOR_FRONT]->pictureW, m_defaultCameraInfo[SENSOR_FRONT]->pictureH,
                  m_curCameraInfo[SENSOR_FRONT]->previewW,     m_curCameraInfo[SENSOR_FRONT]->previewH,
                  (void *)&m_camera_info[SENSOR_FRONT].dummy_shot) == false) {
        CLOGE("ERR(%s):m_setZoom() fail", __func__);
    }

    /* jpeg */
    shot->ctl.jpeg.quality = m_jpegQuality;
    shot->ctl.jpeg.thumbnailSize[0] = m_curCameraInfo[SENSOR_FRONT]->thumbnailW;
    shot->ctl.jpeg.thumbnailSize[1] = m_curCameraInfo[SENSOR_FRONT]->thumbnailH;
    shot->ctl.jpeg.thumbnailQuality = m_jpegThumbnailQuality;
    shot->ctl.jpeg.gpsCoordinates[0] = 0;
    shot->ctl.jpeg.gpsCoordinates[1] = 0;
    shot->ctl.jpeg.gpsCoordinates[2] = 0;
    ...
    /* user request */
    m_camera_info[SENSOR_FRONT].default_shot.dis_bypass = 1;
    m_camera_info[SENSOR_FRONT].default_shot.dnr_bypass = 1;
    m_camera_info[SENSOR_FRONT].default_shot.fd_bypass  = 1;

    m_camera_info[SENSOR_FRONT].default_shot.request_3ax = 1;
    m_camera_info[SENSOR_FRONT].default_shot.request_isp = 1;
    m_camera_info[SENSOR_FRONT].default_shot.request_scc = 0;
    m_camera_info[SENSOR_FRONT].default_shot.request_scp = 1;
    m_camera_info[SENSOR_FRONT].default_shot.request_dis = 0;

    /* default shot */
    memcpy(&m_camera_info[SENSOR_FRONT].dummy_shot, &m_camera_info[SENSOR_FRONT].default_shot, sizeof(struct camera2_shot_ext));
    memcpy(&m_camera_info[SENSOR_FRONT].is3aa_dm, &m_camera_info[SENSOR_FRONT].default_shot, sizeof(struct camera2_shot_ext));
    memcpy(&m_camera_info[SENSOR_FRONT].isp_dm, &m_camera_info[SENSOR_FRONT].default_shot, sizeof(struct camera2_shot_ext));

    return true;
}

bool ExynosCamera::deinitSensor2(void)
{
    CLOGD("[%s] (%d)", __func__, __LINE__);
...
bool ExynosCamera::openSensor2()
{
    if (0 <= m_camera_info[SENSOR_FRONT].sensor.fd) {
        CLOGE("ERR(%s): Already open fd (%d)", __func__, m_camera_info[SENSOR_FRONT].sensor.fd);
        return false;
    }
    ...
    m_camera_info[SENSOR_FRONT].sensor.fd = exynos_v4l2_open(node_name, O_RDWR, 0);

    CLOGD("[%s] (%d) fd %d", __func__, __LINE__, m_camera_info[SENSOR_FRONT].sensor.fd);

    if (m_camera_info[SENSOR_FRONT].sensor.fd < 0) {
        CLOGE("ERR(%s): failed to open sensor2 video node (%s) fd (%d)",
            __func__, node_name, m_camera_info[SENSOR_FRONT].sensor.fd);
        m_camera_info[SENSOR_FRONT].sensor.fd = -1;
        return false;
    }
...
bool ExynosCamera::closeSensor2()
{
    CLOGD("[%s] (%d)", __func__, __LINE__);

    if (0 <= m_camera_info[SENSOR_FRONT].sensor.fd) {
        if (exynos_v4l2_close(m_camera_info[SENSOR_FRONT].sensor.fd) != 0) {
            CLOGE("ERR(%s): sensor exynos_v4l2_close failed", __func__);
            return false;
        }
        m_camera_info[SENSOR_FRONT].sensor.fd = -1;
...
bool ExynosCamera::openIsp2(void)
{
    if (0 <= m_camera_info[SENSOR_FRONT].isp.fd) {
        CLOGE("ERR(%s): Already open fd (%d)", __func__, m_camera_info[SENSOR_BACK].isp.fd);
        return false;
    }
    ...
    m_camera_info[SENSOR_FRONT].isp.fd = exynos_v4l2_open(node_name, O_RDWR, 0);

    CLOGD("[%s] (%d) fd %d", __func__, __LINE__, m_camera_info[SENSOR_FRONT].isp.fd);

    if (m_camera_info[SENSOR_FRONT].isp.fd < 0) {
        CLOGE("ERR(%s): failed to open isp video node (%s) fd (%d)",
            __func__, node_name, m_camera_info[SENSOR_FRONT].isp.fd);
        return false;
    }
...
bool ExynosCamera::closeIsp2(void)
{
    CLOGD("[%s] (%d)", __func__, __LINE__);

    if (0 <= m_camera_info[SENSOR_FRONT].isp.fd) {
        if (exynos_v4l2_close(m_camera_info[SENSOR_FRONT].isp.fd) != 0) {
            CLOGE("ERR(%s): isp exynos_v4l2_close failed", __func__);
            return false;
        }
...
bool ExynosCamera::openPreview2(void)
{
    if (0 <= m_camera_info[SENSOR_FRONT].preview.fd) {
        CLOGE("ERR(%s): Already open fd (%d)", __func__, m_camera_info[SENSOR_BACK].preview.fd);
        return false;
    }
    ...
    m_camera_info[SENSOR_FRONT].preview.fd = exynos_v4l2_open(node_name, O_RDWR, 0);

    CLOGD("[%s] (%d) fd %d", __func__, __LINE__, m_camera_info[SENSOR_FRONT].preview.fd);

    if (m_camera_info[SENSOR_FRONT].preview.fd < 0) {
        CLOGE("DEBUG(%s): failed to open preview video node (%s) fd (%d)",
            __func__, node_name, m_camera_info[SENSOR_FRONT].preview.fd);
        m_camera_info[SENSOR_FRONT].preview.fd = -1;
        return false;
    }
...
bool ExynosCamera::closePreview2(void)
{
    CLOGD("[%s] (%d)", __func__, __LINE__);

    if (0 < m_camera_info[SENSOR_FRONT].preview.fd) {
        if (exynos_v4l2_close(m_camera_info[SENSOR_FRONT].preview.fd) != 0) {
            CLOGE("ERR(%s): exynos_v4l2_close failed", __func__ );
            return false;
        }
...
bool ExynosCamera::openPicture2(void)
{
    if (0 <= m_camera_info[SENSOR_FRONT].picture.fd) {
        CLOGE("ERR(%s): Already open fd (%d)", __func__,m_camera_info[SENSOR_BACK].picture.fd);
        return false;
    }
    ...
    m_camera_info[SENSOR_FRONT].picture.fd = exynos_v4l2_open(node_name, O_RDWR, 0);

    CLOGD("[%s] (%d) fd %d", __func__, __LINE__, m_camera_info[SENSOR_FRONT].picture.fd);

    if (m_camera_info[SENSOR_FRONT].picture.fd < 0) {
        CLOGE("ERR(%s): failed to open picture video node (%s) fd (%d)",
            __func__, node_name, m_camera_info[SENSOR_FRONT].picture.fd);
        m_camera_info[SENSOR_FRONT].picture.fd = -1;
        return false;
    }
...
bool ExynosCamera::closePicture2(void)
{
    CLOGD("[%s] (%d)", __func__, __LINE__);

    if (0 < m_camera_info[SENSOR_FRONT].picture.fd) {
        if (exynos_v4l2_close(m_camera_info[SENSOR_FRONT].picture.fd) != 0) {
            CLOGE("ERR(%s): exynos_v4l2_close failed", __func__);
            return false;
        }
...
void ExynosCamera::m_pushSensorQ2(int index)
{
    Mutex::Autolock lock(m_requestMutex2);
    m_sensorQ2.push_back(index);
}

int ExynosCamera::m_popSensorQ2(void)
{
   List<int>::iterator sensor_token;
   int index;
    ...
    if (m_sensorQ2.size() == 0)
        return -1;

    sensor_token = m_sensorQ2.begin()++;
    index = *sensor_token;
    m_sensorQ2.erase(sensor_token);

    return (index);
}

void ExynosCamera::m_releaseSensorQ2(void)
{
    List<int>::iterator r;

    Mutex::Autolock lock(m_requestMutex2);
    CLOGV("(%s)m_sensorQ2.size : %d", __FUNCTION__, m_sensorQ2.size());

    while (m_sensorQ2.size() > 0) {
        r = m_sensorQ2.begin()++;
        m_sensorQ2.erase(r);
    }
    return;
...
bool ExynosCamera::getSensorBufLpzsl(ExynosBuffer *buf)
{

    return true;
}

bool ExynosCamera::putSensorBufLpzsl(ExynosBuffer *buf)
{
    if (m_flagCreate == false) {
        CLOGE("ERR(%s):Not yet Created", __func__);
        return false;
    ...
    shot_ext = (struct camera2_shot_ext *)(m_camera_info[SENSOR_BACK].isp.buffer[buf->reserved.p].virt.extP[1]);
    shot_ext2 = (struct camera2_shot_ext *)(m_camera_info[SENSOR_FAKE].isp.buffer[buf->reserved.p].virt.extP[1]);

    memcpy(shot_ext2, shot_ext, sizeof(struct camera2_shot_ext));

    shot_ext2->request_scc = m_camera_info[SENSOR_FAKE].dummy_shot.request_scc;
